...
기능 구현이 완료되면 반드시 다음 순서로 작업을 진행합니다:
...
| 단계 | 목적 | 실패 시 조치 |
|------|------|-------------|
| 유닛 테스트 | 개별 함수/컴포넌트 동작 검증 | 코드 수정 후 재실행 |
| E2E 개발 테스트 | 로컬 환경에서 전체 플로우 검증 | 통합 문제 수정 후 재실행 |
| 푸시 & 배포 | 코드 반영 및 프로덕션 배포 | - |
| E2E 프로덕션 테스트 | 실제 환경에서 정상 동작 확인 | 핫픽스 후 재배포 |

### 주의사항

- **테스트 실패 시 절대 푸시하지 않습니다**
- E2E 개발 테스트 전에 개발 서버가 실행 중이어야 합니다
- 프로덕션 테스트는 배포 완료 후 1-2분 대기 후 실행합니다 (배포 반영 시간)
