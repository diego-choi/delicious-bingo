...
- [ ] 다크 모드
- [x] 첫 방문 사용자 온보딩 모달 (빙고 흐름 안내 스텝 모달)
- [ ] 난이도(1/3/5/12 라인) 설명 툴팁 (템플릿 상세 페이지)
- [ ] 리뷰 작성 중 이탈 방지 (`beforeunload` + React Router blocker)
- [ ] 리뷰 최소 글자 수 사전 안내 (입력 전 "최소 10자" 힌트 표시)
- [ ] 페이지 이동 시 스크롤 최상단 이동
- [ ] 페이지 전환 애니메이션 (fade-in)
- [ ] 좋아요/댓글 실패 시 에러 토스트 (silent failure 해소)
- [ ] 비로그인 사용자 좋아요 클릭 시 로그인 유도 메시지
- [ ] 빙고 셀 첫 클릭 시 안내 힌트 ("클릭하여 리뷰를 작성하세요")
- [ ] 댓글 글자 수 제한 표시 (500자 제한 사전 안내)
...
- [ ] 그룹 빙고 챌린지
- [ ] PWA / 오프라인 지원
- [ ] 빙고판 진행률 통계 (프로필에 전체 진행률, 카테고리별 방문 통계 시각화)
